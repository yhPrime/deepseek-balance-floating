  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      slots.inject('sidebar.footer.action', () => slots.register(
        { name: 'sidebar.footer.action', id: 'balance-floating', order: -10, priority: -10 },
        renderPanel,
      ))

      // 复用 dsh-cost-meter 的 cell id 并渲染空:隐藏其在左下角的「当日费用/余额」显示。
      // 必须用更低 priority 才能 shadow 掉 cost-meter 在 priority 0 的同一 id 条目
      // (同 id 同 priority 会被列表槽拒绝,导致插件加载失败)。
      slots.inject('sidebar.footer.action', () => slots.register(
        { name: 'sidebar.footer.action', id: 'cost-meter', order: 0, priority: -10 },
        () => null,
      ))
    }
